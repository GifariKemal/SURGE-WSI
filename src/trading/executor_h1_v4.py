"""Trade Executor H1 v4 - Enhanced with Market Condition Filters
================================================================

Based on analysis of losing months (Feb, Sep, Nov 2025):
- Added Choppiness Index filter (skip > 61.8 = ranging)
- Added ADX filter (skip < 20 = weak trend)
- Changed to ATR-based Stop Loss (adaptive to volatility)
- Added Thursday position size reduction (0% WR in losing months)
- Added regime confidence threshold

Improvements over v3:
1. Market condition filter to avoid ranging/weak markets
2. ATR-based SL instead of fixed 25 pips
3. Thursday caution (position reduced or skipped)
4. Regime confidence threshold (65%)

Research References:
- LuxAlgo: Choppiness Index < 38.2 = trending, > 61.8 = ranging
- FOREX.com: ADX > 20 indicates strong trend
- Academic: Day-of-week effect in FX markets (Thursday 0% WR in losing months)
...
from ..utils.market_condition_filter import MarketConditionFilter, MarketCondition
...
class TradeExecutorH1V4:
    """H1 Trading Executor v4 - Enhanced with Market Condition Filters"""

    def __init__(
        self,
        symbol: str = "GBPUSD",
        warmup_bars: int = 100,
        magic_number: int = 20250130  # Updated for v4
    ):
        """Initialize H1 v4 Executor with enhanced filters"""
        self.symbol = symbol
        ...
        # NEW: Market Condition Filter (v4.2 balanced - tested in backtest)
        # v4.0 was too aggressive (234 filtered), v4.1 too loose (3 filtered)
        # v4.2 balanced: 80 filtered, 89 trades, 2 losing months (tiny losses)
        self.market_filter = MarketConditionFilter(
            chop_ranging_threshold=65.0,      # Skip if choppiness > 65 (balanced)
            adx_weak_threshold=18.0,          # Skip if ADX < 18 (balanced)
            atr_sl_multiplier=1.5,            # SL = ATR * 1.5
            min_sl_pips=15.0,                 # Minimum SL
            max_sl_pips=40.0,                 # Maximum SL
            regime_confidence_threshold=60.0, # Skip if confidence < 60% (balanced)
            enable_thursday_filter=True,
            thursday_position_multiplier=0.6, # Reduce position 60% on Thursday
            skip_thursday_in_weak_market=True, # Skip Thursday if weak market
            min_confluence_score=45.0         # Moderate threshold
        )

        # v4 parameters (ATR-based SL instead of fixed)
        self.tp_rr = 1.5  # 1.5R (same as v3)
        self.base_risk = 0.01  # 1%
        self.min_quality = 0  # No quality filter (like v2)

        # Fallback SL if market filter fails
        self._fallback_sl_pips = 25.0
        ...
        logger.info("H1 v4 Executor initialized with market condition filters")
    ...
    async def process_tick(
        ...
        # =====================================================================
        # NEW in v4: MARKET CONDITION FILTER
        # - Choppiness Index > 61.8 = Skip (ranging market)
        # - ADX < 20 = Skip (weak trend)
        # - Thursday + weak market = Skip
        # - ATR-based SL instead of fixed
        # =====================================================================
        regime_confidence = regime_info.confidence if hasattr(regime_info, 'confidence') else 80.0

        market_condition = self.market_filter.analyze(
            df=self._h1_data,
            current_time=now,
        ...
        # Skip if market conditions unfavorable
        if not market_condition.can_trade:
            self.stats.signals_filtered += 1
            logger.debug(f"Market filter SKIP: {market_condition.condition.value} | Reasons: {market_condition.reasons}")
            return None

        # Get ATR-based SL (adaptive to volatility)
        sl_pips = market_condition.suggested_sl_pips
        tp_pips = sl_pips * self.tp_rr

        # Get Thursday position multiplier
        thursday_multiplier = market_condition.thursday_multiplier

        # =====================================================================
        # END MARKET CONDITION FILTER
        # =====================================================================
        ...
        # LAYER 6: RISK MANAGEMENT (Quality-adjusted + Thursday-adjusted)
        quality_multiplier = 0.8 + (poi['quality'] / 100) * 0.4  # 0.8 to 1.2
        risk_pct = self.base_risk * quality_multiplier

        # Apply Thursday position reduction
        risk_pct = risk_pct * thursday_multiplier
        ...
        # Log market condition info
        logger.info(
            f"Signal PASS: {direction} | {market_condition.condition.value} | "
            f"Chop={market_condition.choppiness:.1f} | ADX={market_condition.adx:.1f} | "
            f"SL={sl_pips:.1f}p | Thu={thursday_multiplier:.0%}"
        )
        ...
        if result.success:
            ...
            # Send Telegram notification with market condition info
            if self.send_telegram:
                market_info = {
                ...
                await self._send_trade_notification(result, session, regime_info.regime.value, market_info)
    ...
    async def _send_trade_notification(self, result: TradeResult, session: str, regime: str, market_info: dict = None):
        """Send trade notification to Telegram"""
        # Calculate pips from actual SL/TP
        if result.direction == 'BUY':
            sl_pips = (result.entry_price - result.stop_loss) / 0.0001
            tp_pips = (result.take_profit - result.entry_price) / 0.0001
        else:
            sl_pips = (result.stop_loss - result.entry_price) / 0.0001
            tp_pips = (result.entry_price - result.take_profit) / 0.0001

        msg = f"🚀 <b>H1 v4 - {result.direction}</b>\n\n"
        ...
        # Add market condition info if available
        if market_info:
            msg += f"\n📊 <b>Market Conditions</b>\n"
            msg += f"├ Choppiness: {market_info.get('choppiness', 0):.1f}\n"
            msg += f"├ ADX: {market_info.get('adx', 0):.1f}\n"
            msg += f"└ ATR SL: {market_info.get('atr_sl', 0):.1f}p"

        msg += f"\n\n⏳ Target: 1.5R (ATR-based)"
    ...
    def get_status(self) -> Dict:
        ...
        # Get last market condition values
        market_values = self.market_filter.get_last_values()

        return {
            'state': self.state.value,
            'strategy': 'H1 v4 (Enhanced Filters)',
            ...
            'market_condition': {
                'choppiness': market_values.get('choppiness', 0),
                'adx': market_values.get('adx', 0),
                'atr_pips': market_values.get('atr_pips', 0),
                'filters': {
                    'chop_threshold': self.market_filter.chop_ranging_threshold,
                    'adx_threshold': self.market_filter.adx_weak_threshold,
                    'thursday_multiplier': self.market_filter.thursday_position_multiplier,
                }
            }
        }
