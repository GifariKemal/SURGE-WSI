"""SURGE-WSI Main - H1 v5.8 Best PF Strategy
============================================

Production-ready H1 trading system based on backtest results:
- 96 trades, 47.9% WR, +$2,269, PF 1.77
- Return: +22.7% in 13 months
- Only 1 losing month

Usage:
    python main_h1_v5_8.py [--demo] [--live] [--interval 300]
...
from src.trading.executor_h1_v5_8 import TradeExecutorH1V58, ExecutorState


class SurgeWSIH1V58:
    """H1 v5.8 Trading Application - Best PF Version"""
    ...
    def __init__(self, mode: str = "demo", verbose: bool = False):
        ...
        setup_logger(
            log_level=log_level,
            log_file=f"logs/surge_h1_v5_8_{mode}.log",
            console=True
        )

        logger.info("=" * 60)
        logger.info(f"SURGE-WSI H1 v5.8 Starting - Mode: {mode.upper()}")
        logger.info("=" * 60)
        ...
        self.executor = TradeExecutorH1V58(
    ...
    async def _telegram_status(self):
        ...
        msg = TF.header("H1 v5.8 Status", TF.CHART)
        msg += TF.spacer()
        msg += TF.item("Strategy", "H1 v5.8 (Best PF)")
    ...
    async def _telegram_test_buy(self):
        ...
        try:
            result = await self.mt5.place_market_order(
                ...
                sl=0, tp=0, comment="H1v58_TEST", magic=20250130
            )
    ...
    async def warmup(self) -> bool:
        ...
        h1_data = self.mt5.get_ohlcv(config.trading.symbol, "H1", 200)
    ...
    async def _send_hourly_status(self, price: float, account: dict):
        ...
        try:
            status = self.executor.get_status()
            msg = TF.header(f"H1 v5.8 - {now.strftime('%H:%M')} UTC", TF.CLOCK)
            msg += TF.spacer()
    ...
    async def run(self, interval_seconds: int = 300):
        ...
        if self.telegram:
            msg = TF.header("H1 v5.8 Started", TF.ROCKET)
            msg += TF.spacer()
    ...
    async def shutdown(self):
        ...
        if self.telegram:
            ...
            msg = TF.header("H1 v5.8 Stopped", TF.WARNING)
            msg += TF.spacer()
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="SURGE-WSI H1 v5.8")
    ...
    app = SurgeWSIH1V58(mode=mode, verbose=args.verbose)
