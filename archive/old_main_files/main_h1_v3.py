"""SURGE-WSI Main - H1 v3 FINAL Strategy
========================================
...
Usage:
    python main_h1_v3.py [--demo] [--live] [--interval 60]
...
from src.trading.executor_h1_v3 import TradeExecutorH1V3, ExecutorState

import MetaTrader5 as mt5


class SurgeWSIH1V3:
    """H1 v3 FINAL Trading Application"""
    ...
    def __init__(self, mode: str = "demo", verbose: bool = False):
        ...
        setup_logger(
            log_level=log_level,
            log_file=f"logs/surge_h1_v3_{mode}.log",
            console=True
        )

        logger.info("=" * 60)
        logger.info(f"SURGE-WSI H1 v3 FINAL Starting - Mode: {mode.upper()}")
        logger.info("=" * 60)
        ...
        # H1 v3 FINAL Executor
        self.executor = TradeExecutorH1V3(
            symbol=config.trading.symbol,
            warmup_bars=100,
            magic_number=20250129
        )
    ...
    async def _telegram_status(self):
        ...
        msg = TF.header("H1 v3 FINAL Status", TF.CHART)
        msg += TF.spacer()
        msg += TF.item("Strategy", "H1 v3 FINAL")
    ...
    async def _send_hourly_status(self, price: float, account: dict):
        ...
        try:
            ...
            msg = TF.header(f"H1 v3 - {now.strftime('%H:%M')} UTC", TF.CLOCK)
            msg += TF.spacer()
    ...
    async def run(self, interval_seconds: int = 60):
        ...
        if self.telegram:
            msg = TF.header("H1 v3 FINAL Started", TF.ROCKET)
            msg += TF.spacer()
    ...
    async def shutdown(self):
        ...
        if self.telegram:
            ...
            msg = TF.header("H1 v3 FINAL Stopped", TF.WARNING)
            msg += TF.spacer()
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="SURGE-WSI H1 v3 FINAL")
    ...
    app = SurgeWSIH1V3(mode=mode, verbose=args.verbose)
