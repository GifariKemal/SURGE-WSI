"""
Z-SCORE MEAN REVERSION FILTER TEST
==================================

Testing Z-Score as a filter for the v6.8 GBPUSD H1 strategy.

Z-Score Formula:
    Z = (Price - SMA) / StdDev

Interpretation for MEAN REVERSION:
    - Z > 2.0: Price extremely overbought (good for SELL - expect reversion down)
    - Z < -2.0: Price extremely oversold (good for BUY - expect reversion up)

Filter Logic:
    - BUY signals: Only allow if Z-Score < -threshold (oversold)
    - SELL signals: Only allow if Z-Score > threshold (overbought)

Testing thresholds: 1.0, 1.5, 2.0
...
# ============================================================
# ZSCORE FILTER CONFIG - TESTING PARAMETERS
# ============================================================
USE_ZSCORE_FILTER = True  # Enable Z-Score filter
ZSCORE_PERIOD = 20        # Period for SMA and StdDev calculation
ZSCORE_THRESHOLD = 1.5    # Default threshold (will test 1.0, 1.5, 2.0)
...
def calculate_zscore(df: pd.DataFrame, col_map: dict, period: int = 20) -> pd.Series:
    ...
    zscore = (close - sma) / (std + 1e-10)  # Avoid division by zero
    return zscore


def check_zscore_filter(zscore_value: float, direction: str, threshold: float = 1.5) -> Tuple[bool, str]:
    """
    Check if trade should be allowed based on Z-Score mean reversion logic.

    Mean Reversion Logic:
    - BUY: Only when Z-Score < -threshold (oversold, expect price to revert UP)
    - SELL: Only when Z-Score > threshold (overbought, expect price to revert DOWN)

    Args:
        zscore_value: Current Z-Score
    ...
    if direction == 'BUY':
        if zscore_value < -threshold:
            return True, f"OVERSOLD_Z={zscore_value:.2f}"
        else:
            return False, f"NOT_OVERSOLD_Z={zscore_value:.2f}"
    else:  # SELL
        if zscore_value > threshold:
            return True, f"OVERBOUGHT_Z={zscore_value:.2f}"
        else:
            return False, f"NOT_OVERBOUGHT_Z={zscore_value:.2f}"
...
# ============================================================
# BACKTEST WITH ZSCORE FILTER
# ============================================================
def run_backtest_zscore(df: pd.DataFrame, col_map: dict, zscore_threshold: float = 1.5) -> Tuple[List[Trade], float, dict, dict]:
    """Run backtest with Z-Score mean reversion filter"""
    trades = []
    ...
    skip_stats = {'MONTH_STOPPED': 0, 'DAY_STOPPED': 0, 'DYNAMIC_ADJ': 0, 'PATTERN_STOPPED': 0, 'ZSCORE_FILTERED': 0}
    ...
    zscore_stats = {
        ...
        'zscore_passed': 0,
        'zscore_blocked': 0,
        'zscore_at_entry': [],  # List of Z-scores at entry
        'blocked_zscores': [],  # List of Z-scores that were blocked
    }
    ...
    for i in range(100, len(df)):
        ...
        # Layer 4: Reset filter if month changed
        ...
        for poi in pois:
            ...
            # ============================================================
            # Z-SCORE FILTER CHECK (NEW!)
            # ============================================================
            zscore_stats['total_signals'] += 1

            if USE_ZSCORE_FILTER:
                zscore_allowed, zscore_reason = check_zscore_filter(
                    current_zscore, poi['direction'], zscore_threshold
                )
            ...
            # Quality check with pattern filter adjustment
...
def print_results(stats: dict, trades: List[Trade], zscore_stats: dict, zscore_threshold: float, baseline_stats: dict = None):
    print(f"\n{'='*70}")
    print(f"Z-SCORE FILTER TEST RESULTS (Threshold: {zscore_threshold})")
    print(f"{'='*70}")
    ...
    print(f"  Filter Logic (Mean Reversion):")
    print(f"    BUY: Only when Z < -{zscore_threshold} (oversold)")
    print(f"    SELL: Only when Z > {zscore_threshold} (overbought)")
    ...
    if zscore_stats['zscore_at_entry']:
        ...
        bins = [(-float('inf'), -2), (-2, -1.5), (-1.5, -1), (-1, 0), (0, 1), (1, 1.5), (1.5, 2), (2, float('inf'))]
        print(f"\n  Z-Score Histogram:")
    ...
    if zscore_stats['blocked_zscores']:
        blocked = zscore_stats['blocked_zscores']
        print(f"\n  Blocked Z-Score Distribution:")
        print(f"    Min: {min(blocked):.2f}")
        print(f"    Max: {max(blocked):.2f}")
        print(f"    Mean: {np.mean(blocked):.2f}")
    ...
    if baseline_stats:
        ...
        # Verdict
        print(f"\n{'='*70}")
        if pnl_diff > 0 and lm_diff <= 0:
            print(f"[VERDICT] IMPROVEMENT - Z-Score filter INCREASED profit by ${pnl_diff:+,.0f}")
            if stats['losing_months'] == 0:
                print(f"[VERDICT] ZERO LOSING MONTHS maintained!")
            verdict = "KEEP"
        elif pnl_diff < 0:
            print(f"[VERDICT] DECLINE - Z-Score filter REDUCED profit by ${abs(pnl_diff):,.0f}")
            verdict = "REJECT"
        elif lm_diff > 0:
            print(f"[VERDICT] WORSE - Z-Score filter ADDED {lm_diff} losing month(s)")
            verdict = "REJECT"
        else:
            print(f"[VERDICT] NEUTRAL - No significant change")
            verdict = "NEUTRAL"
...
async def main():
    ...
    print(f"Z-SCORE MEAN REVERSION FILTER TEST")
    print(f"{'='*70}")
    ...
    # Baseline: v6.8.0 stats (from your context)
    baseline_stats = {
        'total_trades': 115,
        'win_rate': 49.6,
        'profit_factor': 5.09,
    ...
    # Test thresholds: 1.0, 1.5, 2.0
    thresholds = [1.0, 1.5, 2.0]
    results = []

    for threshold in thresholds:
        print(f"\n{'='*70}")
        print(f"Testing Z-Score threshold: {threshold}")
        print(f"{'='*70}")

        trades, max_dd, condition_stats, skip_stats, entry_stats, zscore_stats = run_backtest_zscore(
            df, col_map, zscore_threshold=threshold
        )

        if not trades:
            print(f"No trades for threshold {threshold}")
            continue

        stats = calculate_stats(trades, max_dd)
        print_results(stats, trades, zscore_stats, threshold, baseline_stats)
        ...
        results.append({
            ...
            'filtered_pct': (zscore_stats['zscore_blocked'] / zscore_stats['total_signals'] * 100) if zscore_stats['total_signals'] > 0 else 0
        })

    # Summary comparison
    print(f"\n{'='*70}")
    print(f"Z-SCORE FILTER TEST SUMMARY")
    print(f"{'='*70}")
    print(f"\nBaseline (v6.8.0 without Z-Score filter):")
    ...
    print(f"\nZ-Score Filter Results:")
    print(f"  {'Threshold':>10} {'Trades':>8} {'WR':>7} {'PF':>7} {'P/L':>12} {'Losing':>8} {'Filtered':>10}")
    print(f"  {'-'*10} {'-'*8} {'-'*7} {'-'*7} {'-'*12} {'-'*8} {'-'*10}")

    for r in results:
        print(f"  {r['threshold']:>10.1f} {r['trades']:>8} {r['win_rate']:>6.1f}% {r['profit_factor']:>7.2f} ${r['net_pnl']:>+10,.0f} {r['losing_months']:>8} {r['filtered_pct']:>9.1f}%")

    # Best result
    if results:
        best = max(results, key=lambda x: x['net_pnl'] if x['losing_months'] == 0 else -float('inf'))
        print(f"\n{'='*70}")
        print(f"FINAL RECOMMENDATION")
        print(f"{'='*70}")

        if best['net_pnl'] > baseline_stats['net_pnl'] and best['losing_months'] == 0:
            print(f"[KEEP] Best threshold: {best['threshold']}")
            print(f"       Improvement: ${best['net_pnl'] - baseline_stats['net_pnl']:+,.0f}")
        else:
            print(f"[REJECT] Z-Score mean reversion filter does NOT improve the strategy")
            print(f"         The filter conflicts with the trend-following nature of the strategy")
            print(f"         Mean reversion filters work best in ranging markets, not trending")
