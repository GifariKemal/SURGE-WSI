...
Strategy:
- Trade in direction of EMA 50 slope (trend)
- Enter on pullback to EMA 21 (entry)
...
class MomentumPullbackBacktester:
    """
    Momentum Pullback Strategy:
    - Uptrend: EMA50 sloping up, price pulls back to EMA21, enter BUY
    - Downtrend: EMA50 sloping down, price pulls back to EMA21, enter SELL
    """
    ...
    def __init__(
        ...
        min_adx: float = 18.0,
        ema_slope_periods: int = 5,
        pullback_tolerance: float = 0.0003  # 3 pips tolerance
    ):
        ...
        self.ema_slope_periods = ema_slope_periods
        ...
        self.last_signal_bar = -10  # Track last signal to avoid clustering
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add EMA slope
        df['ema_50_slope'] = (df['ema_50'] - df['ema_50'].shift(self.ema_slope_periods)) / df['ema_50'].shift(self.ema_slope_periods)

        return df

    def generate_signal(self, i: int, df: pd.DataFrame) -> str:
        """
        Momentum pullback signal:
        - BUY: EMA50 sloping up + price near EMA21 from above + RSI not overbought
        - SELL: EMA50 sloping down + price near EMA21 from below + RSI not oversold
        """
        bar = df.iloc[i]
        ...
        ema_21 = bar.get('ema_21', close)
        ema_50 = bar.get('ema_50', close)
        ema_50_slope = bar.get('ema_50_slope', 0)
        ...
        dist_to_ema21 = (close - ema_21) / ema_21

        # Cooldown: at least 3 bars since last signal
        if i - self.last_signal_bar < 3:
            return None

        # UPTREND PULLBACK BUY
        if ema_50_slope > 0.0001:  # EMA50 sloping up
            if close > ema_50:  # Price above EMA50 (uptrend confirmed)
                # Price pulled back near EMA21 (within tolerance)
                if abs(dist_to_ema21) < self.pullback_tolerance * 10:
                    # Previous bar was further from EMA21 (pullback happening)
                    if prev_close > close or prev_close > ema_21 * 1.001:
                        if rsi < 65:  # Not overbought
                            self.last_signal_bar = i
                            return 'BUY'

        # DOWNTREND PULLBACK SELL
        if ema_50_slope < -0.0001:  # EMA50 sloping down
            if close < ema_50:  # Price below EMA50 (downtrend confirmed)
                # Price pulled back near EMA21 (within tolerance)
                if abs(dist_to_ema21) < self.pullback_tolerance * 10:
                    # Previous bar was further from EMA21 (pullback happening)
                    if prev_close < close or prev_close < ema_21 * 0.999:
    ...
    def calculate_position(self, regime: int, atr_pips: float) -> tuple:
        ...
        sl_mult = self.SL_MULT.get(regime, 1.2)
        tp_mult = self.TP_MULT.get(regime, 1.6)
    ...
    def run(self, df: pd.DataFrame) -> dict:
        logger.info(f"Starting MOMENTUM PULLBACK backtest: {len(df)} bars")
        ...
        stats = {'skipped_hour': 0, 'skipped_no_signal': 0, 'skipped_daily': 0}
        ...
        for i in range(51, len(df)):
            ...
            direction = self.generate_signal(i, df)
    ...
    def _calc_stats(self, filter_stats: dict) -> dict:
        ...
        close_stats = {'TP': {'count': 0, 'pnl': 0}, 'SL': {'count': 0, 'pnl': 0}, 'END': {'count': 0, 'pnl': 0}}
        for t in self.trades:
    ...
    def print_results(self, stats: dict):
        print("\n" + "=" * 70)
        print("MOMENTUM PULLBACK BACKTEST")
        print("=" * 70)
        ...
        print("\n" + "-" * 70)
        print("STRATEGY: Momentum Pullback to EMA21")
        print("-" * 70)
        print(f"Entry: Pullback to EMA21 in trend direction")
        print(f"Trend: EMA50 slope + price position")
        ...
        print("-" * 70)
        for r, s in stats['close_stats'].items():
            if s['count'] > 0:
                print(f"{r}: {s['count']} trades, ${s['pnl']:+.2f}")
        ...
        for reg, s in stats['regime_stats'].items():
            ...
            print(f"{reg:<12} {s['trades']:>3} trades | WR: {wr:>5.1f}% | ${s['pnl']:>+8.2f}")
        ...
        for m in sorted(stats['monthly_pnl'].keys()):
            ...
            cum += p
            print(f"{m}  {t:>3} trades  ${p:>+8.2f}  (cum: ${cum:>+8.2f})")
