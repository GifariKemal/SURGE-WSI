"""
Aroon Oscillator Test
=====================
Test Aroon Oscillator for mean reversion.

Aroon measures time since last high/low.
...
- > +50: Strong uptrend
- < -50: Strong downtrend
- Near 0: Ranging/consolidating

For mean reversion: extreme values may indicate reversal.

Sources:
- Tushar Chande (creator)
...
def calculate_aroon(high, low, period=25):
    ...
    aroon_up = high.rolling(period + 1).apply(lambda x: (period - (period - x.argmax())) / period * 100, raw=True)
    aroon_down = low.rolling(period + 1).apply(lambda x: (period - (period - x.argmin())) / period * 100, raw=True)
    aroon_osc = aroon_up - aroon_down
    return aroon_osc, aroon_up, aroon_down
...
def main():
    ...
    print('=' * 100)
    print('AROON OSCILLATOR TEST')
    print('=' * 100)
    print('Formula: Aroon Osc = Aroon Up - Aroon Down')
    print('Range: -100 to +100')
    ...
    def backtest(use_aroon=False, period=25, os_level=-50, ob_level=50):
        if use_aroon:
            aroon_osc, _, _ = calculate_aroon(df['high'], df['low'], period)
            indicator = aroon_osc
        else:
            indicator = df['rsi']
        ...
        for i in range(200, len(df)):
            ...
            if not position:
                ...
                if use_aroon:
                    # Extreme negative = oversold (reversal up expected)
                    # Extreme positive = overbought (reversal down expected)
    ...
    baseline_ret, baseline_trades, baseline_wr, baseline_dd = backtest(use_aroon=False)
    print(f'\n1. v3.7 Baseline RSI(10) 42/58: +{baseline_ret:.1f}% | Trades: {baseline_trades} | WR: {baseline_wr:.1f}% | MaxDD: {baseline_dd:.1f}%')

    print('\n2. AROON OSCILLATOR CONFIGURATIONS')
    print('-' * 80)
    print(f'   {"Config":<30} | {"Return":^10} | {"Diff":^8} | {"Trades":^8} | {"WR":^8} | {"MaxDD":^8}')
    print('   ' + '-' * 90)

    configs = [
        (25, -50, 50, 'Aroon(25) -50/+50 std'),
        (25, -70, 70, 'Aroon(25) -70/+70'),
        (25, -80, 80, 'Aroon(25) -80/+80 extreme'),
        (25, -30, 30, 'Aroon(25) -30/+30 loose'),
        (14, -50, 50, 'Aroon(14) -50/+50'),
        (14, -70, 70, 'Aroon(14) -70/+70'),
        (10, -50, 50, 'Aroon(10) -50/+50 fast'),
        (10, -70, 70, 'Aroon(10) -70/+70'),
        (7, -50, 50, 'Aroon(7) -50/+50 very fast'),
    ]

    for period, os, ob, name in configs:
        ret, trades, wr, max_dd = backtest(use_aroon=True, period=period, os_level=os, ob_level=ob)
