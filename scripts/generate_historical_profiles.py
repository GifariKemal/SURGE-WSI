#!/usr/bin/env python3
"""
Generate Historical Market Profiles (2020-2023)

Extends the market analysis to include historical data for better AI/Bot training.
Generates profiles for 2020-2023 and merges with existing 2024-2026 data.

Total coverage after running: 7 years (2020-2026)
"""
...
class HistoricalAnalyzer:
    ...
    def _get_col(self, df: pd.DataFrame, col: str) -> str:
        """Get correct column name (handles case)"""
...
async def generate_year_profiles(analyzer: HistoricalAnalyzer, year: int,
                                  all_data: pd.DataFrame) -> tuple:
    """Generate profiles for a specific year"""
    print(f"\n  Processing {year}...")

    start = datetime(year, 1, 1, tzinfo=timezone.utc)
    end = datetime(year, 12, 31, 23, 59, 59, tzinfo=timezone.utc)

    # Filter data for this year (with some lookback)
...
async def main():
    ...
    print("=" * 70)
    print("HISTORICAL MARKET PROFILE GENERATOR")
    print("Generating 2020-2023 profiles for AI/Bot training")
    print("=" * 70)
    ...
    try:
        ...
        print("\n[1/3] Fetching all historical H1 data...")
        start = datetime(2019, 12, 1, tzinfo=timezone.utc)  # Extra for lookback
        ...
        # Load existing data (2024-2026)
        print("\n[4/4] Merging with existing 2024-2026 data...")
        ...
        # Convert new profiles to dict format
        new_monthly = {}
        ...
        # Save
        print(f"\nSaving merged data...")
        print(f"  Monthly: {len(new_monthly)} (2020-2023) + {len(existing_monthly)} (2024-2026) = {len(merged_monthly)}")
        print(f"  Daily: {len(new_daily)} (2020-2023) + {len(existing_daily)} (2024-2026) = {len(merged_daily)}")
    ...
    print("COMPLETE!")
    print("=" * 70)
    print("\nData now covers: January 2020 - January 2026 (7 years)")
