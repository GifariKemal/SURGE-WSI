"""Detailed H1 Backtest v5.9.5 - HYBRID (Balanced Frequency + Quality)
=========================================================================

v5.9.5 HYBRID - Balance between frequency and profitability

TARGET: ~0.7-1.0 trades/day with PF > 1.3

APPROACH:
1. MODERATE TRADING HOURS
   - Activity threshold: 45 (between 35 and 55)
   - Allow hybrid trading but with quality check

2. MODERATE QUALITY REQUIREMENTS
   - Min quality: 40 (between 30 and 50)
   - Keep some filtering for bad setups

3. PRICE ACTION FALLBACK
   - Use price action when regime unclear
   - But require stronger price movement (5 pips vs 3 pips)

4. KEEP POI REQUIREMENT
   - Still require POI but with lower quality
   - Better than no POI at all

5. MODERATE RISK
   - 0.7% base risk (between 0.5% and 1%)
   - Balance exposure and profit potential

Version history:
- v5.8:       96T, 47.9% WR, +$2,269, PF 1.77, 0.24/day [QUALITY]
- v5.9 HF:   510T, 41.4% WR, +$399, PF 1.05, 1.30/day [FREQUENCY]
- v5.9.5:    TARGET ~300T, ~44% WR, PF 1.3+, 0.7-1.0/day [HYBRID]
...
class AdaptiveRiskScorer:
    ...
    # v5.9.5 HYBRID: Day multipliers - balanced
    DAY_MULTIPLIERS = {
        0: 1.0,    # Monday
        1: 0.95,   # Tuesday
        2: 1.0,    # Wednesday
        3: 0.6,    # Thursday - moderate penalty
        4: 0.6,    # Friday - moderate penalty
        5: 0.0,    # Saturday - SKIP
        6: 0.0,    # Sunday - SKIP
    }

    # v5.9.5 HYBRID: Moderate quality requirements
    DAY_QUALITY_BONUS = {
        0: 0,      # Monday
        1: 3,      # Tuesday
        2: 0,      # Wednesday
        3: 10,     # Thursday - moderate
        4: 10,     # Friday - moderate
        5: 99,     # Saturday - skip
        6: 99,     # Sunday - skip
    }

    # v5.9.5 HYBRID v3: Focus on quality hours only
    HOUR_MULTIPLIERS = {
        7: 0.9, 8: 0.85,            # Early London - slightly penalized
        9: 1.2,                      # Best hour
        10: 0.6, 11: 0.9,           # Transition
        12: 0.3,                     # SKIP - 27% WR
        13: 1.1, 14: 1.1,           # Overlap - best
        15: 0.7,                     # Post-overlap
        16: 0.3,                     # SKIP - 29% WR
        17: 0.4, 18: 0.4, 19: 0.4, 20: 0.4,  # Late NY - reduced
        21: 0.35, 22: 0.35, 23: 0.4,  # Off hours
        0: 0.35, 1: 0.25, 2: 0.35, 3: 0.3,  # Asia
        4: 0.25, 5: 0.35,
        6: 0.15,                     # SKIP - 20% WR
    }

    # v5.9.5 HYBRID: Moderate hour quality requirements
    HOUR_QUALITY_BONUS = {
        9: -5, 13: -5, 14: -5,      # Best hours
        10: 8, 11: 8,               # Transition
        15: 8, 16: 10,              # Post-overlap
        17: 10, 18: 10, 19: 10, 20: 10,  # Late NY
        22: 15, 1: 20, 4: 20,       # Off hours
    }

    # v5.9.5 HYBRID v3: Stricter entry requirements
    ENTRY_MULTIPLIERS = {
        'MOMENTUM': 1.0,
        'LOWER_HIGH': 0.85,          # Good pattern
        'HIGHER_LOW': 0.3,           # SKIP - 18% WR
        'ENGULF': 0.9,
        'REJECTION': 0.9,            # Good pattern
        'SMALL_BODY': 0.4,           # Risky
        'ANY_BULLISH': 0.3,          # SKIP - fallback
        'ANY_BEARISH': 0.3,          # SKIP - fallback
    }

    # v5.9.5 HYBRID: Moderate entry quality requirements
    ENTRY_QUALITY_BONUS = {
        'MOMENTUM': 0,
        'LOWER_HIGH': 3,
        'HIGHER_LOW': 5,
    ...
    # v5.9.5 HYBRID v3: Higher skip threshold for better quality
    SKIP_THRESHOLD = 0.26  # Between HF (0.15) and v5.8 (0.28)

    def __init__(self,
                 min_combined_score: float = 42.0,  # v5.9.5 v3: Higher quality bar
                 min_risk_multiplier: float = 0.22,  # v5.9.5 v3: Slightly higher min
    ...
    def calculate_risk_score(
        ...
        # v5.9.5 HYBRID: Session multiplier - moderate
        session_mult = 1.0 if in_killzone else 0.7  # Between 0.65 and 0.8

        # v5.9.5 HYBRID: Drift multiplier - moderate penalty
        drift_mult = 0.7 if drift_detected else 1.0  # Between 0.6 and 0.8
...
def run_backtest(df: pd.DataFrame) -> tuple:
    """Run v5.9.5 HYBRID backtest - balanced frequency & quality"""

    # Initialize components
    killzone = KillZone()
    activity_filter = DynamicActivityFilter(
        min_atr_pips=4.0,           # v5.9.5: Between 3.0 and 5.0
        min_bar_range_pips=2.5,     # v5.9.5: Between 2.0 and 3.0
        activity_threshold=30.0,    # v5.9.5: Between 25.0 and 35.0
        pip_size=0.0001
    )
    activity_filter.outside_kz_min_score = 45.0  # v5.9.5: Between 35.0 and 55.0

    market_filter = MarketConditionFilter(
        chop_ranging_threshold=68.0,  # v5.9.5: Between 65.0 and 70.0
        adx_weak_threshold=14.0,      # v5.9.5: Between 12.0 and 16.0
        atr_sl_multiplier=1.5, min_sl_pips=15.0, max_sl_pips=40.0,
        regime_confidence_threshold=50.0,  # v5.9.5: Between 45.0 and 55.0
        enable_thursday_filter=False,
        min_confluence_score=35.0     # v5.9.5: Between 30.0 and 40.0
    )

    # V5.9.5 HYBRID: Adaptive Risk Scorer with moderate thresholds
    risk_scorer = AdaptiveRiskScorer(
        min_combined_score=38.0,  # v5.9.5: Between 30.0 and 45.0
        min_risk_multiplier=0.20, # v5.9.5: Between 0.15 and 0.3
        max_risk_multiplier=1.3
    )

    drift_detector = ADWINDriftDetector(delta=0.005, min_samples=50)
    session_manager = SessionProfileManager(hybrid_mode=True)

    # v5.9.5 HYBRID: Confluence validator - moderate
    confluence_validator = ConfluenceValidator(
        min_total_score=42,    # v5.9.5: Between 35 and 48
        min_factors_passed=3   # Keep 3 for more trades
    )

    # v5.9.5 HYBRID: Volatility Regime Filter - moderate
    volatility_filter = VolatilityRegimeFilter(
        atr_lookback=50,
        low_vol_threshold=0.68,        # v5.9.5: Between 0.60 and 0.75
        very_low_vol_threshold=0.48,   # v5.9.5: Between 0.40 and 0.55
        high_chop_threshold=65.0,      # v5.9.5: Between 60.0 and 70.0
        min_quality_score=36.0,        # v5.9.5: Between 30.0 and 42.0
        reduced_risk_threshold=54.0,   # v5.9.5: Between 50.0 and 58.0
    )
    ...
    # v5.9.5 HYBRID: Moderate cooldowns - balance frequency & protection
    cooldown_after_sl = timedelta(minutes=45)  # Between 30min (HF) and 60min (v5.8)
    cooldown_after_tp = timedelta(minutes=20)  # Between 15min (HF) and 30min (v5.8)
    ...
    for idx in range(100, len(df)):
        ...
        # v5.9.5 HYBRID: Base risk 0.7% - balance between 0.5% (HF) and 1% (v5.8)
        base_risk = 0.007
...
def print_report(stats, final_balance, trades):
    ...
    print()
    print("=" * 70)
    print("H1 BACKTEST v5.9.5 HYBRID RESULTS - BALANCED FREQUENCY & QUALITY")
    print("=" * 70)
    print()
    ...
    print("=" * 70)
    print("v5.8:     96T, 47.9% WR, +$2,269, PF 1.77, 0.24/day [QUALITY]")
    print("v5.9 HF: 510T, 41.4% WR, +$399,  PF 1.05, 1.30/day [FREQUENCY]")
    print(f"v5.9.5: {stats.total_trades:>4}T, {stats.win_rate:.1f}% WR, ${stats.total_pnl:+,.0f}, PF {stats.profit_factor:.2f}, {stats.trades_per_day:.2f}/day [HYBRID]")

    # Target check
    if stats.profit_factor >= 1.3 and stats.trades_per_day >= 0.7:
        print(">>> TARGET MET: PF >= 1.3 and >= 0.7 trades/day")
    elif stats.profit_factor >= 1.3:
        print(">>> PF target met, but trades/day below 0.7")
    elif stats.trades_per_day >= 0.7:
        print(">>> Trades/day target met, but PF below 1.3")
    print("=" * 70)
...
async def send_telegram_report(stats, trades, final_balance):
    ...
    try:
        ...
        msg = f"""<b>H1 BACKTEST v5.9.5 HYBRID</b>
<b>Balanced Frequency & Quality</b>
...
<b>COMPARISON</b>
v5.8: 96T, 0.24/day, PF 1.77
v5.9 HF: 510T, 1.30/day, PF 1.05
<b>v5.9.5: {stats.total_trades}T, {stats.trades_per_day:.2f}/day, PF {stats.profit_factor:.2f}</b>
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("SURGE-WSI H1 BACKTEST v5.9.5 HYBRID")
    print("Balanced Frequency & Quality System")
    print("=" * 70)
