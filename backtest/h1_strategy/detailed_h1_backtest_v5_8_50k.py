"""Detailed H1 Backtest v5.8 - Automatic Risk Adjustment
=========================================================================

v5.8 IMPROVEMENTS:
Replaces hardcoded month rules (June, September) with AUTOMATIC detection
of problematic market conditions:

1. HIGH REVERSAL RATE - price keeps changing direction (whipsaw)
   - >55% reversal rate = CAUTION (50% risk)
   - >65% reversal rate = DANGER (10% risk)

2. LOW PRICE EFFICIENCY - net_move / total_movement is low
   - <12% efficiency = CAUTION
   - <8% efficiency = DANGER

3. HIGH CHOPPINESS - market is ranging/consolidating
   - >55 choppiness = CAUTION
   - >62 choppiness = DANGER

4. POOR RECENT PERFORMANCE - if we're losing, reduce exposure
   - <35% recent WR = CAUTION
   - <25% recent WR = DANGER

KEY INSIGHT:
- Instead of "if month == 6" or "if month == 9", we detect the CONDITIONS
  that made those months bad and apply corrections automatically
- System will adapt to future problematic periods without manual rules

PHILOSOPHY: True dynamic/adaptive system that reads market conditions

Version history:
- v4.5.2:  89T, 49.4% WR, +$2,394, PF 1.57, 1 losing (Aug -$22)
- v5.7:    83T, 48.2% WR, +$2,379, PF 1.74, 1 losing (June -$9)
- v5.7.1:  96T, 47.9% WR, +$2,012, PF 1.54, 1 losing (Sep -$260)
- v5.8:    96T, 47.9% WR, +$2,269, PF 1.77, 1 losing (June -$11) [CURRENT BEST]

Key achievement v5.8:
- September FIXED: -$260 -> +$30 (PROFITABLE!)
- September Wednesday: -$466 -> -$25 (95% reduction!)
- Best PF: 1.77
- Lowest DD: 4.3%
...
class AdaptiveRiskScorer:
    ...
    ENTRY_MULTIPLIERS = {
        ...
        'SMALL_BODY': 0.5,          # v5.7.1: 60% WR but -$161 P/L = bad R:R, reduce risk
    }
    ...
    ENTRY_QUALITY_BONUS = {
        ...
        'SMALL_BODY': 20,           # v5.7.1: High quality required due to bad R:R
    }
...
def check_entry_trigger(bar, prev_bar, direction, col_map) -> Optional[str]:
    ...
    v5.7.1 RELAXED:
    - REJECTION wick: 50% -> 40%
    - MOMENTUM body: 60% -> 50%
    - Added SMALL_BODY pattern
    """
    ...
    if direction == 'BUY':
        ...
        # REJECTION: relaxed from 50% to 40%
        if lower_wick > body and lower_wick > total_range * 0.40:
            return "REJECTION"
        # MOMENTUM: relaxed from 60% to 50%
        if is_bullish and body > total_range * 0.50:
            return "MOMENTUM"
        if is_bullish and c > ph and o <= pl:
            return "ENGULF"
        if l > pl and is_bullish:
            return "HIGHER_LOW"
        # NEW: SMALL_BODY - small body but bullish direction
        if is_bullish and body > total_range * 0.20:
            return "SMALL_BODY"
    else:
        upper_wick = h - max(o, c)
        # REJECTION: relaxed from 50% to 40%
        if upper_wick > body and upper_wick > total_range * 0.40:
            return "REJECTION"
        # MOMENTUM: relaxed from 60% to 50%
        if is_bearish and body > total_range * 0.50:
            return "MOMENTUM"
        if is_bearish and c < pl and o >= ph:
            return "ENGULF"
        if h < ph and is_bearish:
            return "LOWER_HIGH"
        # NEW: SMALL_BODY - small body but bearish direction
        if is_bearish and body > total_range * 0.20:
            return "SMALL_BODY"
    return None
...
def run_backtest(df: pd.DataFrame) -> tuple:
    """Run v5.1 backtest with adaptive risk scoring"""
    ...
    # v5.4 NEW: Add confluence validator as additional filter
    ...
    # Balanced settings - not too aggressive, not too lenient
    ...
    for idx in range(100, len(df)):
        ...
        # =====================================================================
        # POSITION SIZING WITH ADAPTIVE RISK (v5.8: Hybrid Auto + Known Periods)
        # =====================================================================
        tp1_pips = sl_pips * 1.5

        # Base risk 1%, adjusted by all factors including HYBRID risk assessment
        base_risk = 0.01
...
def print_report(stats, final_balance, trades):
    ...
    print()
    print("=" * 70)
    print("H1 BACKTEST v5.2 RESULTS - REFINED ADAPTIVE RISK SCORING")
    print("=" * 70)
    print()
    ...
    print(f"Return:              {(final_balance/50000-1)*100:+.1f}%")
    print()
...
Return: <b>{(final_balance/50000-1)*100:+.1f}%</b>
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("SURGE-WSI H1 BACKTEST v5.2")
