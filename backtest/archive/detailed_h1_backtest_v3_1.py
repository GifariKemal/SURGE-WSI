"""Detailed H1 Backtest v3.1 - Optimized Research Improvements
==============================================================

Changes from v3:
- Keep: Ensemble Regime Detection (improved WR by 3%)
- Keep: Advanced Rejection Candle (64% WR)
- Keep: LOWER_HIGH entry (56% WR)
- Remove: MOMENTUM entry (50% WR, negative P/L)
- Remove: HIGHER_LOW entry (30% WR, negative P/L)
- Relax: Quality filter from 50 to 35
- Change: Use fixed SL/TP instead of ATR (simpler, more consistent)
- Change: Use base risk instead of full Kelly (more stable)

Target: Better than v2 (106 trades, 48.1% WR, +$2,018)
...
# ============================================================================
# ENSEMBLE REGIME DETECTION (from v3)
# ============================================================================
...
# ============================================================================
# ENHANCED OB QUALITY (from v3, simplified)
# ============================================================================
...
def calculate_enhanced_ob_quality(
    ...
    """Enhanced OB quality - simplified version"""
...
# ============================================================================
# ADVANCED REJECTION CANDLE (from v3)
# ============================================================================
...
# ============================================================================
# ENTRY TRIGGER (Optimized - only profitable types)
# ============================================================================

def check_entry_trigger_optimized(
    bar: pd.Series,
    prev_bars: pd.DataFrame,
    direction: str,
    col_map: dict
    ...
    """
    Entry types - ONLY profitable ones from v3 analysis:
    - REJECTION (64% WR, +$240)
    - LOWER_HIGH for SELL (56% WR, +$84)
    - HIGHER_LOW for BUY removed (30% WR, negative)
    - MOMENTUM removed (50% WR, negative)
    """
    o = bar[col_map['open']]
    h = bar[col_map['high']]
    l = bar[col_map['low']]
    ...
    if direction == 'BUY':
        # 1. Advanced rejection (best performer)
        is_rej, rej_conf = detect_advanced_rejection(bar, prev_bars, direction, col_map)
        if is_rej:
            return "REJECTION", rej_conf

        # 2. Strong bullish momentum ONLY if very strong (body > 70%)
        if is_bullish and body > total_range * 0.7:
            return "MOMENTUM", 55
        ...
        # 2. Lower high + bearish close (profitable in v3)
        if h < ph and is_bearish:
            return "LOWER_HIGH", 55

        # 3. Strong bearish momentum ONLY if very strong
        if is_bearish and body > total_range * 0.7:
            return "MOMENTUM", 55
...
def run_backtest_v3_1(df: pd.DataFrame, use_hybrid: bool = True) -> tuple:
    """Run backtest v3.1 with optimized parameters"""
    ...
    for idx in range(100, len(df)):
        ...
        # Detect POI with enhanced quality
        ...
        # Quality filter - RELAXED from 50 to 35
        if poi['quality'] < 35:
            continue
        ...
        # Check entry trigger (optimized - only profitable types)
        entry_type, entry_confidence = check_entry_trigger_optimized(bar, prev_bars, direction, col_map)
        if not entry_type:
            continue

        combined_quality = (poi['quality'] + entry_confidence) / 2

        # Fixed SL/TP (simpler, more consistent)
        sl_pips = 25.0
        tp1_pips = sl_pips * 1.5  # 1.5R

        # Quality-adjusted risk (simpler than Kelly)
        base_risk = 0.01
        quality_mult = 0.8 + (combined_quality / 100) * 0.4  # 0.8 to 1.2
        risk_pct = base_risk * quality_mult
...
def print_report(stats: BacktestStats, final_balance: float, trades: List[BacktestTrade]):
    ...
    print()
    print("=" * 70)
    print("H1 DETAILED BACKTEST v3.1 RESULTS")
    print("(Optimized Research Improvements)")
    print("=" * 70)
    print()

    print("v3.1 OPTIMIZATIONS:")
    print("-" * 50)
    print("+ Ensemble Regime Detection (kept - improved WR)")
    print("+ Advanced Rejection Candle (kept - 64% WR)")
    print("+ LOWER_HIGH for SELL (kept - 56% WR)")
    print("- HIGHER_LOW removed (30% WR, negative P/L)")
    print("- MOMENTUM reduced (50% WR, negative P/L)")
    print("- Quality filter relaxed (35 instead of 50)")
    print("- Fixed SL/TP instead of ATR (simpler)")
    print()
    ...
    print("COMPARISON")
    print("-" * 50)
    print("v2:   106 trades, 48.1% WR, +$2,018, PF 1.22")
    print("v3:    51 trades, 51.0% WR,  -$230, PF 0.84")
    print(f"v3.1: {stats.total_trades:>3} trades, {stats.win_rate:.1f}% WR, ${stats.total_pnl:+,.0f}, PF {stats.profit_factor:.2f}")
    print()
...
async def send_telegram_report(stats: BacktestStats, trades: List[BacktestTrade], final_balance: float):
    ...
    try:
        ...
        msg = f"""*H1 BACKTEST v3.1 (Optimized)*
Period: Jan 2025 - Jan 2026 (13 months)

*v3.1 OPTIMIZATIONS*
+ Ensemble Regime (kept)
+ Advanced Rejection (kept)
+ LOWER_HIGH for SELL (kept)
- HIGHER_LOW removed
- MOMENTUM reduced
...
v3:  51 trades, 51% WR, -$230
v3.1: {stats.total_trades} trades, {stats.win_rate:.0f}% WR, ${stats.total_pnl:+,.0f}
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("SURGE-WSI DETAILED H1 BACKTEST v3.1")
    print("Period: 13 Months (Jan 2025 - Jan 2026)")
    print("Strategy: Optimized Research Improvements")
    print("=" * 70)
    ...
    print("\n[2/3] Running backtest v3.1...")
    trades, stats, final_balance = run_backtest_v3_1(df, use_hybrid=True)
