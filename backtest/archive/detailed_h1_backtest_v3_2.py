"""Detailed H1 Backtest v3.2 - Back to v2 Baseline + Selective Improvements
==========================================================================

Analysis of previous versions:
- v2:   106 trades, 48.1% WR, +$2,018, PF 1.22 (PROFITABLE - baseline)
- v3:    51 trades, 51.0% WR,  -$230, PF 0.84 (too restrictive)
- v3.1:  38 trades, 39.5% WR,   +$19, PF 1.01 (too restrictive)

Problem identified: Ensemble Regime Detection was too restrictive
Solution: Go back to v2's HMM regime + add only validated improvements

v3.2 Strategy:
1. Use original HMM Regime (like v2) - NOT Ensemble
2. All entry types from v2 (REJECTION, MOMENTUM, ENGULF, HIGHER_LOW, LOWER_HIGH)
3. Enhanced OB Quality scoring (from research) - better quality calc
4. NO quality filter threshold (like v2) - don't restrict trades
5. Quality-adjusted position sizing (from research) - higher quality = higher risk

Target: Match or exceed v2 (+$2,018) with better quality signals
...
from src.analysis.regime_detector import HMMRegimeDetector  # Back to original HMM


# ============================================================================
# ENHANCED OB QUALITY (from research - kept as improvement)
# ============================================================================
...
def calculate_enhanced_ob_quality(
    ...
    - Base quality from impulse move (0-50 pts)
    - Wick analysis bonus (0-25 pts)
    - Fresh zone bonus (0-25 pts)
    """

    quality = 0.0

    if ob_idx < 5 or ob_idx >= len(df) - 3:
        return 50  # Default quality if can't calculate
    ...
    if ob_range > 0:
        ...
        if direction == 'BUY':
            # For bullish OB (bearish candle), upper wick shows sellers getting absorbed
            upper_wick = ob_bar[high_col] - max(ob_bar[open_col], ob_bar[close_col])
            wick_ratio = upper_wick / ob_range
        else:
            # For bearish OB (bullish candle), lower wick shows buyers getting absorbed
...
# ============================================================================
# POI DETECTION (v2 style + enhanced quality)
# ============================================================================
        ...
        if direction == 'BUY':
            if bar[close_col] < bar[open_col]:  # Bearish candle
                ...
                if move_up > 0.0010:  # 10 pips
                    # Use enhanced quality calculation
...
def check_entry_trigger(bar: pd.Series, prev_bar: pd.Series, direction: str, col_map: dict) -> Optional[str]:
    ...
    if direction == 'BUY':
        # 1. Rejection candle (best performer)
...
# ============================================================================
# MAIN BACKTEST (v2 structure + quality-adjusted sizing)
# ============================================================================

def run_backtest(df: pd.DataFrame, use_hybrid: bool = True) -> tuple:
    """Run backtest - v2 baseline + quality-adjusted sizing"""

    # Initialize components (same as v2)
    killzone = KillZone()
    ...
    regime_detector = HMMRegimeDetector()  # Original HMM, NOT Ensemble
    ...
    for idx in range(100, len(df)):
        ...
        # Manage open position (same as v2)
        if position:
            # Check SL
        ...
        # Check Kill Zone (same as v2)
        in_kz, session = killzone.is_in_killzone(current_time)

        # Check hybrid mode (same as v2)
        ...
        # Check regime (same as v2 - original HMM)
        ...
        # Detect POI with enhanced quality (v3.2 improvement)
        ...
        # NO quality filter threshold - take all trades like v2

        # Check price near POI zone (same as v2)
        ...
        # Check entry trigger (same as v2 - all types)
        entry_type = check_entry_trigger(bar, prev_bar, direction, col_map)
        if not entry_type:
            continue

        # Calculate position size with QUALITY-ADJUSTED SIZING (v3.2 improvement)
        sl_pips = 25.0
        tp1_pips = sl_pips * 1.5  # 1.5R
        tp2_pips = sl_pips * 2.5  # 2.5R

        # Quality-adjusted risk: 0.8% to 1.2% based on POI quality
        base_risk = 0.01
...
def print_report(stats: BacktestStats, final_balance: float, trades: List[BacktestTrade]):
    ...
    print()
    print("=" * 70)
    print("H1 DETAILED BACKTEST v3.2 RESULTS")
    print("(v2 Baseline + Quality-Adjusted Sizing)")
    print("=" * 70)
    print()

    print("v3.2 APPROACH:")
    print("-" * 50)
    print("+ Original HMM Regime (same as v2)")
    print("+ All entry types (REJECTION, MOMENTUM, ENGULF, etc.)")
    print("+ Enhanced OB Quality scoring (from research)")
    print("+ NO quality filter threshold")
    print("+ Quality-adjusted position sizing (0.8%-1.2%)")
    print()
    ...
    print("v3:     51 trades, 51.0% WR,   -$230, PF 0.84 [too restrictive]")
    print("v3.1:   38 trades, 39.5% WR,    +$19, PF 1.01 [too restrictive]")
    print(f"v3.2: {stats.total_trades:>4} trades, {stats.win_rate:.1f}% WR, ${stats.total_pnl:+,.0f}, PF {stats.profit_factor:.2f}")
    print()
...
async def send_telegram_report(stats: BacktestStats, trades: List[BacktestTrade], final_balance: float):
    ...
    try:
        ...
        msg = f"""*H1 BACKTEST v3.2*
(v2 Baseline + Quality Sizing)
Period: Jan 2025 - Jan 2026 (13 months)

*APPROACH*
+ Original HMM Regime
+ All entry types from v2
+ Enhanced OB Quality
...
v3.2: {stats.total_trades} trades, {stats.win_rate:.0f}% WR, ${stats.total_pnl:+,.0f}
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("SURGE-WSI DETAILED H1 BACKTEST v3.2")
    print("Period: 13 Months (Jan 2025 - Jan 2026)")
    print("Strategy: v2 Baseline + Quality-Adjusted Sizing")
    print("=" * 70)
